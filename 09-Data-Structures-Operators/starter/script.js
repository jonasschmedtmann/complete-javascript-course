...
console.log(`--- String Methods (Flights) --- `);
console.log(flights.split('+'));

const getAirportCode = str => str.slice(0, 3).toUpperCase();

for (const flight of flights.split('+')) {
  const [type, departureLocation, arrivalLocation, duration] =
    flight.split(';');
  const output = `${type.includes('Delayed') ? '🔴' : ''}${type.replaceAll(
    '_',
    ' '
  )} from ${getAirportCode(departureLocation)} to ${getAirportCode(
    arrivalLocation
  )} (${duration.replace(':', 'h')})`.padStart(45, ' ');
  console.log(output);
}
...
//   const text =
//     'underscore_case\nfirst_name\nSome_Variable\ncalculate_AGE\ndelayed_departure';
//   const textArgArr = text.split('\n');
//   console.log(textArgArr);
//   console.log(textArgArr.entries());
//   for (const [i, arg] of textArgArr.entries()) {
//     const [first, second] = arg.toLowerCase().split('_');
