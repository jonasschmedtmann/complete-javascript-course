	...
	calc: function () {
		for (var i = 0; i < this.bills.length; i++) {
			if (bills[i] < 100) {
				...
				this.finals[i] = this.bills[i] + this.bills[i] * 0.2;
			} else if (this.bills[i] >= 100 && this.bills[i] < 300) {
				this.tips[i] = this.bills[i] * 0.1;
				this.finals[i] = this.bills[i] + this.bills[i] * 0.1;
			} else {
				this.tips[i] = this.bills[i] * 0.25;
				this.finals[i] = this.bills[i] + this.bills[i] * 0.25;
			}
		}
	},
};
...
tipCalculator.calc();
console.log(tipCalculator.tips, tipCalculator.finals);
