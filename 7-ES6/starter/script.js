...
const john6 = new Person6('John', 1980, 'consultant');
...
class Park extends Element {
    constructor(name, buildYear, area, numTrees) {
        ...
        this.area = area;   //sq mi
        this.numTrees = numTrees;
    }

    getDensity() {
        const density = this.numTrees / this.area;
        console.log(`${this.name} has a density of ${density} trees per square mile.`)
    }

    getAge() {
        let curYear = new Date().getFullYear();
        return curYear - this.buildYear;
    }
}
...
function reportParks(p) {
    ...
    // Density
    allParks.forEach(el => el.getDensity());

    // Avg Age
    const ages = p.map(el => el.getAge());
    const [totalAge, avgAge] = calc(ages);
    console.log(`Our ${p.length} parks have an average age of ${avgAge} years.`)

    // Parks having > 1000 trees
    const idx = p.map(el => el.numTrees)
